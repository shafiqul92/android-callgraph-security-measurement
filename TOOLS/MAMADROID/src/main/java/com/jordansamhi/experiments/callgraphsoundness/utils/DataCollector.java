...
public class DataCollector {
    ...
    private void saveDataToFile(String appName, String algo, String content) {
        // Basename only → file is created under JVM cwd (Python sets cwd to results dir).
        // Full path would write next to APK (often root-owned 755 → Permission denied for normal users).
        String apkBase = new File(appName).getName();
        String fileName = String.format("%s-%s-callgraph.txt", apkBase, algo);
