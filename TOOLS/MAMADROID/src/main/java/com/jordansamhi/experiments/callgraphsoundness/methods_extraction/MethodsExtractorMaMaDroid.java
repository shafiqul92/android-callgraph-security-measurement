...
public class MethodsExtractorMaMaDroid extends MethodsExtractorBase {
    ...
    public MethodsExtractorMaMaDroid(String apkPath) {
    ...
    protected void buildCallGraph(String algo, String appName) {
        ...
        if (resource == null) {
            throw new IllegalStateException("SourcesAndSinks.txt not found on classpath");
        }
        File sourcesAndSinksFile;
        try {
            sourcesAndSinksFile = File.createTempFile("SourcesAndSinks", ".txt");
            sourcesAndSinksFile.deleteOnExit();
            try (InputStream in = resource.openStream()) {
                Files.copy(in, sourcesAndSinksFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
            }
        } catch (IOException e) {
            throw new RuntimeException("Could not extract SourcesAndSinks.txt from classpath", e);
        }
        ifac.getAnalysisFileConfig().setSourceSinkFile(sourcesAndSinksFile);
        ifac.getAnalysisFileConfig().setTargetAPKFile(new File(appName));
        ifac.getAnalysisFileConfig().setAndroidPlatformDir(new File(platformsPath));
